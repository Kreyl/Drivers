...
void STMPE811_t::Init(i2c_t* pi2c) {
	this->I2C_TOUCH=pi2c;
    // IRQ pin
	PinSetupInput(TOUCH_INT_GPIO, TOUCH_INT_PIN, pudPullDown);
    // Reset
    if(Write(STMPE811_SYS_CTRL1, 0x02) != retvOk) return;
    chThdSleepMilliseconds(5);
    Write(STMPE811_SYS_CTRL1, 0x00);
    chThdSleepMilliseconds(2);
    uint16_t Data;
    if(Read(STMPE811_CHIP_ID, &Data) != retvOk) return;
    Printf("\r tch: %X\r", Data);
...
//        Uart.Printf("tch: %X\r", b);
    ...
    int16_t FX = 0, FY = 0;
    uint8_t Data[3];
    if(Read(STMPE811_TSC_DATA_XYZ, Data, 3) != retvOk) return retvFail;
